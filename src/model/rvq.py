...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, x): # (batch_size, emb_dim, time)
        x_flat = x.permute(0, 2, 1).reshape(-1, self.embedding_dim) # (batch_size*time, emb_dim)
        ...
        if self.training:
            one_hot = F.one_hot(indices, num_classes=self.num_embeddings).type_as(x_flat)

            batch_cluster_counts = one_hot.sum(dim=0)
            batch_embedding_sums = one_hot.T @ x_flat
            ...
            smoothed_cluster_size = (self.cluster_size + self.epsilon) / (total_count + self.num_embeddings * self.epsilon) * total_count
...
class ResidualVectorQuantizer(nn.Module):
    ...
    def __init__(self, num_quantizers, num_embeddings, embedding_dim):
        ...
        self.layers = nn.ModuleList([
            VectorQuantizer(num_embeddings, embedding_dim) for _ in range(num_quantizers)
        ])
    ...
    def forward(self, x):
        ...
        return out, avg_loss, all_indices
