...
class Encoder(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.layers = nn.Sequential(
            ...
            EncoderBlock(in_channels=in_channels, out_channels=2*in_channels, stride=2),
            EncoderBlock(in_channels=2*in_channels, out_channels=4*in_channels, stride=4),
            EncoderBlock(in_channels=4*in_channels, out_channels=8*in_channels, stride=5),
            EncoderBlock(in_channels=8*in_channels, out_channels=16*in_channels, stride=5),
            nn.ELU(),
            CausalConv(
...
class Decoder(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.layers = nn.Sequential(
            CausalConv(in_channels=out_channels, out_channels=16*in_channels, kernel_size=7),
            DecoderBlock(in_channels=16*in_channels, out_channels=8*in_channels, stride=5),
            DecoderBlock(in_channels=8*in_channels, out_channels=4*in_channels, stride=5),
            DecoderBlock(in_channels=4*in_channels, out_channels=2*in_channels, stride=4),
            DecoderBlock(in_channels=2*in_channels, out_channels=in_channels, stride=2),
            nn.ELU(),
            CausalConv(
                in_channels=in_channels, 
                out_channels=1, 
...
class SoundStreamModel(nn.Module):
    ...
    def __init__(self, in_channels=32, out_channels=128, num_quantizers=8, num_embeddings=1024):
        super().__init__()
